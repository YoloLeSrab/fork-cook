...
func _physics_process(delta):
	...
	if scrolling:
		target_position.y = CAMERA_LIMIT - scroll_bar.value
	clamp(target_position.y, 0, CAMERA_LIMIT - 1200)
	global_position = global_position.lerp(target_position, lerp_speed * delta)
	if !scrolling:
		scroll_bar.value = CAMERA_LIMIT - target_position.y
